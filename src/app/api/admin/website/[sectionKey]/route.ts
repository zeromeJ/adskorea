...
export async function PUT(request: Request, context: Context) {
  ...
  const section = await prisma.$transaction(async (tx) => {
    ...
    for (const asset of assets) {
      await tx.websiteAsset.create({ data: {
        sectionKey, itemKey: asset.itemKey, label: asset.label, type: asset.type,
        url: asset.url, originalUrl: asset.originalUrl, storagePath: asset.storagePath,
        originalStoragePath: asset.originalStoragePath, originalFileName: asset.originalFileName,
        storedFileName: asset.storedFileName, mimeType: asset.mimeType, size: asset.size,
        width: asset.width, height: asset.height, cropX: asset.cropX, cropY: asset.cropY,
        cropWidth: asset.cropWidth, cropHeight: asset.cropHeight, zoom: asset.zoom,
        rotation: asset.rotation, aspectRatio: asset.aspectRatio, outputWidth: asset.outputWidth,
        outputHeight: asset.outputHeight, sortOrder: asset.sortOrder ?? 0,
        published: asset.published ?? true, metadata: asset.metadata,
      }});
    }
