      ...
      <div className="mx-auto grid min-w-0 max-w-[1200px] gap-8 lg:grid-cols-[0.76fr_1.24fr] lg:items-start lg:gap-10">
        ...
        <form className="min-w-0 max-w-full rounded-lg bg-white p-5 sm:p-8" noValidate onSubmit={handleSubmit}>
          <div className="grid min-w-0 grid-cols-1 gap-x-5 gap-y-5 md:grid-cols-2 md:[&>*]:min-w-0">
            ...
            <Input aria-required="true" autoComplete="email" id="email" label="이메일 (필수)" maxLength={254} onChange={(event) => updateField("email", event.target.value)} placeholder="name@company.com" required type="email" value={formData.email} />
          ...
          <div className="mt-5 min-w-0 rounded-lg bg-[var(--muted-surface)] p-4" id="privacy-details">
            ...
            <details className="mt-3 min-w-0 text-xs leading-6 text-[var(--sub-text)]"><summary className="cursor-pointer font-bold text-[var(--primary)]">수집·이용 내용 보기</summary><div className="mt-2 [overflow-wrap:anywhere]"><p><strong>수집 항목:</strong> 회사명, 담당자명, 이메일, 연락처 및 사용자가 선택적으로 입력한 문의 내용·첨부파일·상세정보</p><p><strong>이용 목적:</strong> 문의 확인, 제품 상담, 견적 검토, 자료 제공, 회신 및 고객 응대</p><p><strong>보유기간:</strong> 문의 처리 목적 달성 후 지체 없이 파기하며, 관계 법령에 따라 보존이 필요한 경우 해당 기간 동안 보관합니다.</p><p>동의를 거부할 수 있으나 문의 접수가 제한될 수 있습니다.</p></div></details>
